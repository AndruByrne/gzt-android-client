...
final public class UserActionHandlers {
    ...
    public void showStats(View view) {
        ...
        if (!gztOverlayAnimator.recoverView(
                BottomNav.STATS,
    ...
    public void showMap(View view) {
        ...
        if (instance.isGooglePlayServicesAvailable(view.getContext()) != ConnectionResult.SUCCESS) {
            ...
            if (!gztOverlayAnimator.recoverView(
                    BottomNav.MAP,
                    activityBinding.gztSettingsContentFrame)) {
                GZTMapComponent mapComponent = application.createMapComponent(); //  create map
                // component and assign it to the map view
                PowerUpsMapViewBinding mapBinding = PowerUpsMapViewBinding.inflate(
                        context.getLayoutInflater(),
                        mapComponent);
                gztOverlayAnimator.replaceFrameContentsWith(
                        activityBinding.gztSettingsContentFrame,
                        mapBinding.extranetMapView,
                        BottomNav.MAP);
            }
        }
    }
