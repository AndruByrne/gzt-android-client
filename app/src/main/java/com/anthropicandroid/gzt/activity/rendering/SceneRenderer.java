...
package com.anthropicandroid.gzt.activity.rendering;
...
import com.anthropicandroid.gzt.activity.GZTSettingsView;
 ...
 * interact with {@link ViewGroup} while in VR.
 */
public final class SceneRenderer {
    ...
    @Nullable
    private final ViewGroup androidLayout;
    @Nullable
    ...
    /* package */ SceneRenderer(
            CanvasQuad canvasQuad, ViewGroup androidLayout, Handler uiHandler,
            SurfaceTexture.OnFrameAvailableListener externalFrameListener) {
        this.canvasQuad = canvasQuad;
        this.androidLayout = androidLayout;
     ...
     * initializing the object on the GL thread.
     *
     * <p>The also creates a {@link GZTSettingsView} that is bound to the VR scene. The View is backed by
     * a {@link CanvasQuad} and is meant to be rendered in a VR scene.
     *
     * @param context the {@link Context} used to initialize the {@link GZTSettingsView}
     * @param parent  the new view is attached to the parent in order to properly handle Android
     *                events
     * @return a SceneRender configured for VR and a bound {@link GZTSettingsView} that can be treated
     * similar to a View returned from findViewById.
     */
    @MainThread
    public static Pair<SceneRenderer, GZTSettingsView> createForVR(Context context, ViewGroup parent) {
        CanvasQuad canvasQuad = new CanvasQuad();
        GZTSettingsView gztSettingsView = GZTSettingsView.createForOpenGl(context, parent, canvasQuad);
        OnFrameAvailableListener externalFrameListener = gztSettingsView.getFrameListener();

        SceneRenderer scene = new SceneRenderer(
                canvasQuad, gztSettingsView, new Handler(Looper.getMainLooper()), externalFrameListener);
    ...
    public void glDrawFrame(float[] viewProjectionMatrix, int eyeType) {
        ...
        if (androidLayout != null) {
            canvasQuad.glDraw(viewProjectionMatrix, androidLayout.getAlpha());
        }
    ...
    /**
     * Processes Daydream Controller clicks and dispatches the event to {@link ViewGroup} as a
    ...
    public void toggleUi() {
        ...
        uiHandler.post(
                new Runnable() {
                    ...
                    public void run() {
                        if (androidLayout.getAlpha() == 0) {
                            androidLayout.animate().alpha(1).start();
                        } else {
                            androidLayout.animate().alpha(0).start();
                        }
                    }
                });
    }
